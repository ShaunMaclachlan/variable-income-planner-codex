...
export function calendarEventToShift(event: CalendarShiftInput): Shift | null {
  const title = event.summary.trim()
  const normalisedTitle = title.toLowerCase()
  const isWorkEvent = /phyllis\s*tuckwell/i.test(title) || /syringe\s*pump\s*training/i.test(title)
